...
Like most toolkit functions, listing assignments in Moodle involves two primary steps: loading the test data into a [Moodle Docker image](https://github.com/edulinq/lms-docker-moodle-testdata) and fetching data from Moodle.
...
I needed the grade book page to display the assignments accurately because it contains all the assignments' essential attributes, including the maximum points values.
...
By default, the assignments' maximum points were missing from the grade book page.
I learned that a special setting needed to be enabled in order for the maximum points values to appear.
I investigated the request sent to the server when this setting was toggled on in the browser and worked to mimmic this procedure in my code.
